...
describe('CoopResultQuery', () => {
  test('Validity', () => {
    ...
    for (const file of files) {
      ...
      expect(() =>
        CoopResultQuery.CoopHistory(CoopHistoryDetailQuery.CoopHistoryDetail).parse(JSON.parse(data))
      ).not.toThrow()
    }
  })
  test('Equality', () => {
    ...
    for (const file of files) {
      ...
      const input_model = CoopResultQuery.CoopHistory(CoopHistoryDetailQuery.CoopHistoryDetail).parse(JSON.parse(input))
      const difference = diff(JSON.parse(JSON.stringify(input_model)), JSON.parse(output))
      if (difference !== undefined) {
        console.error(
          JSON.stringify(
            CoopResultQuery.CoopHistory(CoopHistoryDetailQuery.CoopHistoryDetail).parse(JSON.parse(input)),
            null,
            2
          )
        )
